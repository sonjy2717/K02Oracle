...
/*
trunc() : 소수점을 특정 자리수에서 잘라낼 때 사용하는 함수
...
/*
시나리오] 사원 테이블에서 보너스율이 있는 사원만 인출한 후 보너스율을
    소수점 1자리로 표현하시오
*/
-- 1.커미션이 있는 사원들 추출하기
...
round(값, 자리수) : 반올림 처리
    두 번째 인자가
        없는 경우 : 소수점 첫 번째 자리가 5이상이면 올림, 미만이면 버림
        있는 경우 : 숫자만큼 소수점이 표현되므로 그 다음수가 5이상이면 올림,
            미만이면 버림
*/
...
-- 첫 번째 항목 : 소수이하 6자리까지 표현하므로 7을 올림처리하여 0.123457
-- 두 번째 항목 : 소수이하 4자리까지 표현하므로 1을 버림처리하여 2.3456
