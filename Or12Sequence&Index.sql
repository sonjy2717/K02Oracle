...
create sequence 시퀀스명
    ...
    [Minvalue n | NoMinvalue] -> 시퀀스 최소 값 지정 : 디폴트1
    [Maxvalue n | NoMaxvalue] -> 시퀀스 최대 값 지정 : 디폴트1.0000E+28
    [Cycle | NoCycle] -> 최대/최소 값에 도달할 경우 처음부터 다시
                        시작할지 여부를 설정(cycle로 지정하면 최대 값까지
                        증가 후 다시 시작 값부터 재시작됨)
...
1. start with에 minvalue보다 작은 값을 지정할 수 없다. 즉, start with값은
    minvalue와 같거나 커야한다.
2. nocycle로 설정하고 시퀀스를 계속 얻어올 때 maxvalue에 지정 값을
    초과하면 에러가 발생한다.
...
create sequence seq_serial_num
    ...
    cycle               /* 최대값 도달시 시작값부터 재시작 여부 : yes */
    nocache;            /* 캐시사용여부 : no */
...
/*
    다음 입력할 일련번호(시퀀스)를 반환한다. 실행 할 때마다 다음으로
    넘어간다.
*/
...
-- 인덱스 생성하기
/*
    tb_goods테이블의 g_name컬럼에 인덱스를 생성한다.
    인덱스명은 tb_goods_name_idx으로 지정한다.
*/
