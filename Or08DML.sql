...
-- update와 마찬가지로 where절이 없으면 모든 레코드가 삭제된다.
...
1          엄태웅        어른 승민      1975-11-21
2          이제훈       대학생 승민     1978-07-23
3          한가인        어른 서연      1982-10-24
4          배수지       대학생 서연     1988-05-21
*/
insert into pr_emp values(1, '엄태웅', '어른 승민', to_date('1975-11-21'));
insert into pr_emp values(2, '이제훈', '대학생 승민', to_date('1978-07-23'));
insert into pr_emp values(3, '한가인', '어른 서연', to_date('1982-10-24'));
insert into pr_emp values(4, '배수지', '대학생 서연', to_date('1988-05-21'));
...
*/
delete from pr_emp where substr(regist_date, 4, 2) = '10';

/*
6.  pr_emp 테이블을 복사해서 pr_emp_clone 테이블을 생성하되 
다음 조건에 따르시오. 
조건1 : 기존의 컬럼명을 idx, name, nickname, regidate 와같이 
변경해서 복사한다. 
조건2 : 레코드까지 모두 복사한다. 
*/
