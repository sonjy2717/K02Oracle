...
alter table tb_primary3 add constraint tb_primary3_pk
    primary key (user_name); -- 제약명을 부여하면 오류발생시 해당 이름을 통해 확인 가능
desc tb_primary3;
select * from user_constraints;
...
insert into tb_unique (idx, name, telephone, nickname)
    values (1, '예리', '010-3333-3333', ''); -- 에러. 중복된 idx값
    
insert into tb_unique values (4, '정우성', '010-4444-4444', '영화배우');
insert into tb_unique values (5, '이정재', '010-5555-5555', '영화배우'); --허용
insert into tb_unique values (6, '황정민', '010-4444-4444', '영화배우'); --실패
/*
    telephone과 nickname 컬럼이 동일한 제약명으로 설정되었으므로
    두 개의 컬럼이 동시에 동일한 값을 가지는 경우가 아니라면 중복된 값이
    허용된다.
    즉, 4번과 5번은 서로 다른데이터로 인식하므로 입력되고,
...
- 만약 테이블간에 외래키가 설정되어 있다면 자식테이블에 참조 값이
...
insert into tb_foreign4 values (8, 'ㅎㅇㅎㅇ', 'gasan'); -- 부모키가 없으므로 에러발생

select * from tb_primary4;
select * from tb_foreign4; -- 삽입된 레코드가 있는걸 확인함

delete from tb_primary4; /*
        부모테이블에서 레코드를 삭제할시 on delete cascade 옵션때문에
...
않고 참조키 부분만 null값으로 변경되어 더 이상 참조할 수 없는
레코드로 변경된다.
*/
...
insert into tb_default values ('cccc', ''); -- null 입력됨
insert into tb_default values ('dddd', ' '); -- 공백(space) 입력됨
...
배정되지 않도록 외래키 제약조건을 지정하되 
제약조건 이름은 pr_emp_dept_fk 로 지정하시오.
*/
...
insert into pr_emp_const values 
    (100, '아무개', '열정', null, sysdate, 1000, 0.5, 10); -- 입력실패:부모키 없음
insert into pr_emp_const values 
    (200, '머시기', '냉정', null, sysdate, 1000, 0, 10); -- 입력실패:check제약조건 위배
...
insert into pr_emp_const values 
    (200, '머시기', '냉정', null, sysdate, 1000, 0.34, 10);
