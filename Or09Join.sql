...
파일명 : Or09Join.sql
테이블조인
설명 : 두 개 이상의 테이블을 동시에 참조하여 데이터를 가져와야 할때
    사용하는 SQL문
**********************/
...
/*
2] outer join (외부조인)
outer join은 inner join과는 달리 두 테이블에 조인 조건이 정확히
일치하지 않아도 기준이 되는 테이블에서 결과 값을 가져오는 join방식이다.
...
from
    ...
    right outer join locations L
        on D.location_id = L.location_id;
-- left혹은 right와 같이 기준이 되는 테이블이 달라지면 인출되는 결과도 달라진다.
...
형식2(오라클 방식)
    ...
    => 위의 경우 왼쪽 테이블1이 기준이 된다.
*/

/*
시나리오] 전체직원의 사원번호, 이름, 부서아이디, 부서명, 지역을
    외부조인을 통해 출력하시오. 단, 오라클 방식으로 작성하시오.
*/
select
    employee_id, first_name, last_name, D.department_id,
    department_name, city
...
select
    ...
    nvl(department_name, '<부서없음>') dep_name
from employees E 
    left outer join departments D
        on E.department_id = D.department_id
where
...
where
    ...
    and Kimberely.first_name = 'Kimberely' 
    and Kimberely.last_name = 'Grant';
...
where city = initcap('seattle');
/*
    using절에 사용된 식별자 컬럼의 경우 select절에서 테이블의 별칭을
    붙이면 오류가 발생한다.
...
*/
select
    employee_id, first_name, last_name, to_char(salary, '999,000') "급여",
...
where
    ...
    city = initcap('south san francisco');
...
/*
3. 사원의 이름(FIRST_NAME)에 'A'가 포함된 모든사원의 이름과 
부서명을 출력하시오.
출력목록] 사원이름, 부서명
*/
select
...
*/
select
    first_name, last_name, job_title, E.department_id, department_name
from 
...
where
    ...
    city = initcap('toronto') and
    state_province = initcap('ontario');
...
-- 매니저 아이디가 108이므로 사원번호를 조회
select first_name from employees where employee_id = 108;
-- 셀프 조인을 통해서 해당 사원의 매니저 정보를 출력한다.
select 
...
select 
    distinct M.first_name, M.last_name, M.job_id, M.salary
from 
    employees C, employees M
where
    C.manager_id = M.employee_id and
    C.job_id = 'FI_ACCOUNT';
...
select to_char(hire_date, 'yyyy') year, avg(salary)
from employees
    inner join jobs using(job_id)
where
