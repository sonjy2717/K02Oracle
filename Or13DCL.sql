...
select username, default_tablespace from dba_users
    where username in upper('test_user2'); -- 우리가 설정한대로 users를 사용하고 있음

-- users 테이블 스페이스에 10M 공간을 할당
...
SQL> ed new_table [Enter]
    : 이와 같은 명령으로 사용자 계정 디렉토리에 new_table.sql파일이 생성된다.
...
-- test_user2 계정에서 테이블 생성과 시퀀스 생성을 시도 : 시퀀스는 권한 부족으로 에러발생됨.
